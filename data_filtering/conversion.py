...
original_f = open('original.txt','r')
...
for s in original_f :    #result라는 리스트는 원본에서 모든 데이터를 갖고 있음 
    l = s.split('\t')
    result.append(l)
    count=count+1
...
random.shuffle(mom)             
train_data = mom[:number]
test_data = mom[number:]

for i in range(0,len(mom[0])) :
...
for i in range(0,number) :
    for j in range(0,col_count) :
        if(j==0) :
            ...
            s_l[j]=float(s_l[j])+float(train_data[i][j])
            pow_l[j]=float(pow_l[j])+float(train_data[i][j])*float(train_data[i][j])
        ...
        if(j==0) :
            ...
            if(sd_l[j]==0) :
               tx_l[j]=float(0)
                ...
                tx_l[j]=float((float(train_data[i][j])-m_l[j])) / float(sd_l[j])
    ...
    for j in range(0,col_count) :
        if(j==0) :
            ...
            if(sd_l[j]==0):
                ...
                sx_l[j]=float((float(test_data[i][j])-m_l[j])) / float(sd_l[j])
